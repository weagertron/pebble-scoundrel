#include "game_over.h"
#include "../render.h"
#include <stdio.h>

// ──────────────────────────────────────────────
// Static state
// ──────────────────────────────────────────────

static Window *s_window = NULL;
static Layer *s_canvas = NULL;
static GameOverCallback s_callback = NULL;
static bool s_victory = false;
static int16_t s_score = 0;
static uint8_t s_hp = 0;
static int s_selected = 0;

#define MENU_PLAY_AGAIN  0
#define MENU_MAIN        1
#define MENU_COUNT       2

static const char *MENU_LABELS[] = {"Play Again", "Menu"};

// Layout
#define TITLE_Y       40
#define TITLE_H       30
#define SCORE_Y       90
#define SCORE_H       24
#define DETAIL_Y      120
#define DETAIL_H      18
#define MENU_START_Y  155
#define MENU_H        36
#define MENU_MARGIN   20

// ──────────────────────────────────────────────
// Canvas drawing
// ──────────────────────────────────────────────

static void canvas_update_proc(Layer *layer, GContext *ctx) {
  GRect bounds = layer_get_bounds(layer);

  // Background
  graphics_context_set_fill_color(ctx, GColorBlack);
  graphics_fill_rect(ctx, bounds, 0, GCornerNone);

  // Title
  GFont title_font = fonts_get_system_font(FONT_KEY_GOTHIC_28_BOLD);
  if (s_victory) {
    graphics_context_set_text_color(ctx, GColorIslamicGreen);
    graphics_draw_text(ctx, "ESCAPED!", title_font,
                       GRect(0, TITLE_Y, SCREEN_W, TITLE_H),
                       GTextOverflowModeWordWrap, GTextAlignmentCenter, NULL);
  } else {
    graphics_context_set_text_color(ctx, GColorDarkCandyAppleRed);
    graphics_draw_text(ctx, "DEFEATED", title_font,
                       GRect(0, TITLE_Y, SCREEN_W, TITLE_H),
                       GTextOverflowModeWordWrap, GTextAlignmentCenter, NULL);
  }

  // Score
  static char score_buf[20];
  snprintf(score_buf, sizeof(score_buf), "Score: %d", s_score);
  GFont score_font = fonts_get_system_font(FONT_KEY_GOTHIC_24_BOLD);
  graphics_context_set_text_color(ctx, GColorWhite);
  graphics_draw_text(ctx, score_buf, score_font,
                     GRect(0, SCORE_Y, SCREEN_W, SCORE_H),
                     GTextOverflowModeWordWrap, GTextAlignmentCenter, NULL);

  // Detail
  static char detail_buf[32];
  if (s_victory) {
    snprintf(detail_buf, sizeof(detail_buf), "HP remaining: %d", s_hp);
  } else {
    snprintf(detail_buf, sizeof(detail_buf), "HP: 0");
  }
  GFont detail_font = fonts_get_system_font(FONT_KEY_GOTHIC_14);
  graphics_context_set_text_color(ctx, GColorLightGray);
  graphics_draw_text(ctx, detail_buf, detail_font,
                     GRect(0, DETAIL_Y, SCREEN_W, DETAIL_H),
                     GTextOverflowModeWordWrap, GTextAlignmentCenter, NULL);

  // Menu
  for (int i = 0; i < MENU_COUNT; i++) {
    GRect item_rect = GRect(MENU_MARGIN, MENU_START_Y + i * (MENU_H + 4),
                            SCREEN_W - 2 * MENU_MARGIN, MENU_H);
    bool selected = (i == s_selected);

    graphics_context_set_fill_color(ctx, selected ? GColorWhite : GColorDarkGray);
    graphics_fill_rect(ctx, item_rect, 4, GCornersAll);

    graphics_context_set_text_color(ctx, selected ? GColorBlack : GColorWhite);
    GFont menu_font = fonts_get_system_font(FONT_KEY_GOTHIC_18_BOLD);
    graphics_draw_text(ctx, MENU_LABELS[i], menu_font, item_rect,
                       GTextOverflowModeWordWrap, GTextAlignmentCenter, NULL);
  }
}

// ──────────────────────────────────────────────
// Button handling
// ──────────────────────────────────────────────

static void up_handler(ClickRecognizerRef recognizer, void *context) {
  s_selected = (s_selected - 1 + MENU_COUNT) % MENU_COUNT;
  layer_mark_dirty(s_canvas);
}

static void down_handler(ClickRecognizerRef recognizer, void *context) {
  s_selected = (s_selected + 1) % MENU_COUNT;
  layer_mark_dirty(s_canvas);
}

static void select_handler(ClickRecognizerRef recognizer, void *context) {
  if (!s_callback) return;
  switch (s_selected) {
    case MENU_PLAY_AGAIN: s_callback(SCREEN_GAME); break;
    case MENU_MAIN:       s_callback(SCREEN_TITLE); break;
  }
}

static void back_handler(ClickRecognizerRef recognizer, void *context) {
  if (s_callback) s_callback(SCREEN_TITLE);
}

static void click_config_provider(void *context) {
  window_single_click_subscribe(BUTTON_ID_UP, up_handler);
  window_single_click_subscribe(BUTTON_ID_DOWN, down_handler);
  window_single_click_subscribe(BUTTON_ID_SELECT, select_handler);
  window_single_click_subscribe(BUTTON_ID_BACK, back_handler);
}

// ──────────────────────────────────────────────
// Window handlers
// ──────────────────────────────────────────────

static void window_load_handler(Window *window) {
  Layer *root = window_get_root_layer(window);
  GRect bounds = layer_get_bounds(root);

  s_canvas = layer_create(bounds);
  layer_set_update_proc(s_canvas, canvas_update_proc);
  layer_add_child(root, s_canvas);
}

static void window_unload_handler(Window *window) {
  layer_destroy(s_canvas);
  s_canvas = NULL;
  window_destroy(s_window);
  s_window = NULL;
}

// ──────────────────────────────────────────────
// Public API
// ──────────────────────────────────────────────

void game_over_window_push(GameOverCallback callback, bool victory,
                           int16_t score, uint8_t hp) {
  if (s_window) return;

  s_callback = callback;
  s_victory = victory;
  s_score = score;
  s_hp = hp;
  s_selected = 0;

  s_window = window_create();
  window_set_window_handlers(s_window, (WindowHandlers) {
    .load = window_load_handler,
    .unload = window_unload_handler
  });
  window_set_click_config_provider(s_window, click_config_provider);
  window_set_background_color(s_window, GColorBlack);
  window_stack_push(s_window, true);
}

void game_over_window_pop(void) {
  if (s_window) {
    window_stack_remove(s_window, true);
  }
}
